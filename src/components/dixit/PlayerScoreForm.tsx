...
export default function PlayerScoreForm({ players, onAddScore }: ScoreInputProps) {
  ...
  const scoreChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (numberValue > players.length + 1) {
      setInputErrorMessage(`현재 한 라운드당 최대 점수는 ${players.length + 1}점이에요`);
      setScore(0);
      return;
    }
